 ...
 ******************************************************************************/
	completePageRender : function(){
		//페이지 상수 정의
		aichat013.setPageEnum();
		//탭 화면 디자인
		aichat013.defineTab();
		//그리드 디자인
		aichat013.defineGrid();
		//트리뷰 디자인
		aichat013.defineTree();
		//다이얼로그레이어 디자인
		aichat013.defineDialog();
		//파일업다운로드뷰어 디자인
		aichat013.defineFile();
		//초기데이터 설정
		aichat013.initPage();
		//이벤트 정의
		aichat013.defineEvent();
	},
// -----------------------------------------------------------------------
// ENUM: 열거형 class 상수 정의 [기본함수명:setPageEnum]
// -----------------------------------------------------------------------
	setPageEnum : function(){
		aichat013.enum = new Enumeration();
		aichat013.enum.setEnum("name"							,"code"			,"codeName"						,"pcode");
	},
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
	defineGrid : function(){
		$("#divGrid01").XuiGrid({
			...
			colModel: [
			    {name: "col_0", width: 4.85, align: "center", sort: true, hidden: false, excel: true, html: false, format: "", convertFn:aichat013.typeFn},
			    {name: "col_1", width: 12.12, align: "left", sort: true, hidden: false, excel: true, html: false, format: ""},
			    {name: "col_2", width: 21.21, align: "left", sort: true, hidden: false, excel: true, html: false, format: "", convertFn:aichat013.titleFn},
			    ...
			    {name: "col_6", width: 4.85, align: "center", sort: true, hidden: false, excel: true, html: false, format: "", convertFn:aichat013.tooltipFn}
            ],
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
	selectData : function(){
		//데이터유효성 체크
		if(!aichat013.validationSelectData()){return;}

        // TODO:PB [AI비서] UI 확인용 테스트
		var jsonData01 = aichat013.createData01();
 ...
 ****************************************************************************************************************************************************************/
    ...
    tooltipFn:function(rowIdx, rowId, rowData, cellIdx, cellId){
        ...
        returnValue =
            '<div class="xui-grid-button" tabIndex="0" onClick="aichat013.openRecoSummary(this,event)">' +
