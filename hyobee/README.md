...
### 11.7 Java 21과 Virtual Thread 옵션

프로젝트는 `pom.xml` 기준 Java 21을 사용합니다. `HYOBEE_VIRTUAL_THREADS_ENABLED=true`로 설정하면 WRTN blocking REST 호출을 virtual thread executor로 offload할 수 있습니다. 운영 적용 전에는 thread dump, connection pool, timeout, 장애 전파 방식을 함께 검증하세요.
...
| 문서 | 설명 |
| --- | --- |
| `docs/harness/auth-flow-analysis.md` | 로그인, SSO, 세션, JSP gate 분석 |
| `docs/harness/package-scope.md` | 패키지 경계와 테스트 스코프 |
| `docs/harness/qa-registry.md` | QA 실행 결과와 수동 회귀 현황 |
| `docs/harness/todo.md` | 활성 작업과 DoD |
| `docs/harness/README.md` | 에이전트 하네스 운영 개요 |
