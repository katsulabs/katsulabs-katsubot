...
> **원칙:** LLM/RAG 추론은 **katsubot-api 밖 별도 서비스**(`katsulabs-ai-gateway`)로 구성한다.  
> `katsubot-api`는 레거시 `HyobeeChatApiClient` → WRTN 호출과 같이 **HTTP/SSE 클라이언트만** 둔다.
...
| 별도 AI Gateway | 검색·LLM·생성 (`katsulabs-ai-gateway`) | katsubot-api DB 직접 소유(선택) |

## 설정

```yaml
# services/katsubot-api/application.yml
katsubot:
  rag:
...
`RAG_SERVICE_BASE_URL`만 바꿔 스테이징·운영 Gateway로 전환한다. **katsubot-api 코드 변경 최소.**
...
응답의 `sources`는 Phase 2+ — katsubot-api는 Phase 1에서 무시.
...
Phase 2+ RAG 확장: `data: {"source":{...}}` — katsubot-api는 무시 가능 (하위 호환).
...
- `packages/api-contract/` — **katsubot-api ↔ browser** OpenAPI (RAG API 아님)
- 본 문서 — **katsubot-api ↔ RAG 서비스** 계약 (필요 시 `packages/rag-contract/`로 분리 가능)
