...
| 경로 | 역할 | 에이전트 |
|------|------|----------|
| `apps/katsubot-web/` | React + TypeScript + Vite | Frontend |
| `services/katsubot-api/` | Spring Boot 4.1, Clean Architecture | Backend |
...
| **Backend** | Use Case·인프라·API 테스트 | `services/katsubot-api/**` |
| **Frontend** | React UI·SSE·API 클라이언트 | `apps/katsubot-web/**` |
| **QA** | CI·통합 스모크·PR 게이트 | `**/*Test*`, `.github/workflows/**`, `docs/harness/**` |

**순서:** Contract → Backend → Frontend → QA (병렬은 `agent-hierarchy.md` 조건 충족 시)

## 4. Clean Architecture (katsubot-api)
