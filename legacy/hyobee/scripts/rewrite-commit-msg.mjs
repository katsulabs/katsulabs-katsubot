...
const REWRITES = {
  ...
  'docs: include mapper interface cleanup scope': ['docs: mapper 인터페이스 정리 범위 문서 반영'],
  'docs: plan xtrmjsonnode record migration': ['docs: XtrmJsonNode record 마이그레이션 계획'],
  'docs: plan xtrmjson record migration': ['docs: XtrmJson record 마이그레이션 계획'],
  'docs: add Cursor Cloud dev environment instructions for Hyobee': [
    'docs: Hyobee Cursor Cloud 개발 환경 가이드 추가',
  ],
  'docs: write Korean Hyobee README': ['docs: Hyobee README 한글 작성'],
  ...
  '[TB-005][005f] JDK 21 pilot with virtual threads and pooled WRTN HTTP clients': [
    ...
    '컴파일 타깃 21, blocking I/O 가상 스레드 executor, SSE/multipart 공유 WebClient, RestTemplate 풀링 및 stale connection 검사(CLOSE_WAIT 완화).',
  ],
  '[TB-005][005e-9] Remove XtrmJSON and gson dependency (G9)': [
    '[TB-005][005e-9] XtrmJSON·gson 의존성 제거 (G9)',
    '레거시 Gson 타입 삭제, DAO·서비스·뷰·컨트롤러를 Jackson XtrmJsonNode로 이전, pom에서 gson 제거, P0+P1 회귀 통과.',
  ],
  '[TB-005][005e-8] Migrate Excel/View layer and remove GsonHttpMessageConverter': [
    ...
    '파일·Excel export 뷰를 XtrmJsonNode로 이전, MVC Gson converter 제거(Jackson @JsonValue), ApiServiceImpl Gson 주입 제거.',
  ],
  ...
  'feat(TB-005): Phase 005e-4 — migrate xs/vob/cmmn to XtrmJsonNode': [
    ...
    'vob CmmnService를 Jackson 노드로 이전, XtrmJsonNodes DAO 헬퍼 추가, Login·Management 경계 호출부 조정. G3 P0+P1 통과.',
  ],
  'feat(TB-005): Phase 005e-3 — migrate xs/domain to XtrmJsonNode': [
    'feat(TB-005): 005e-3 xs/domain XtrmJsonNode 마이그레이션',
    'Domain 서비스·컨트롤러 XtrmJsonNode 전환, XtrmJsonNodes DAO 브릿지, 레거시 호출부 asJsonNode() 변환. G3 P0+P1 통과.',
  ],
  'feat(TB-005): Phase 005e-2 — XtrmJSON delegates to XtrmJsonNode': [
    'feat(TB-005): 005e-2 XtrmJSON → XtrmJsonNode 위임',
    'XtrmJSON 데이터 경로를 Jackson XtrmJsonNode로 통합, Gson 경계 API 유지, 브릿지·위임 golden 테스트 추가. G3 P0+P1 통과.',
  ],
  ...
  'feat(TB-005): Phase 005d — remove Gson from aichat v2': [
    ...
    '@SerializedName·Gson을 Jackson @JsonProperty·JsonAdapter로 교체, ChatLogService·ChatStreamServiceImpl 포함.',
  ],
  'feat(TB-005): Phase 005c — jjwt 0.12 migration': [
    'feat(TB-005): 005c jjwt 0.12 마이그레이션',
    'jjwt 0.12.6 업그레이드, HyobeeJwtTokenServiceImpl·HyobeeSSOServiceImpl 및 auth 테스트 stub을 신규 builder/parser API로 이전, JWT wire 호환 유지.',
  ],
  'feat(TB-005): Phase 005b — JDK 17 and record pilot DTOs': [
    'feat(TB-005): 005b JDK 17·record 파일럿 DTO',
    'java.version 17, JwtClaims·VobLoginResult record 전환, JwtClaimsTest 추가, HyobeeThumbnailService UTF-8 인코딩(JDK 17 빌드).',
  ],
  'feat(TB-005): Phase 005a — unused deps removal and patch version bumps': [
    'feat(TB-005): 005a 미사용 의존성 제거·패치 버전 상향',
    'org.json·guava·ibatis-sqlmap 제거, Jackson SB BOM 2.13.5 정렬, 패치 라이브러리 bump, TB-005 harness 문서 추가.',
  ],
  'fix(harness): P0+P1 auth gate green and project skills': [
    'fix(harness): P0+P1 인증 게이트 통과 및 프로젝트 skill 추가',
    'LoginServiceImplTest loginHyobeeSSO·AuthPagePreloadTest JSP 게이트 수정, 한글 skill 17개·qa-registry·harness-status 문서 추가.',
  ],
  'refactor(TB-004): extract aichat010 Phase 1a IIFE modules': [
    'refactor(TB-004): aichat010 Phase 1a IIFE 모듈 분리',
    'hyobee-constants·hyobee-i18n·hyobee-api 추가, aichat010 facade 위임, main.jsp 로드 순서 갱신.',
  ],
...
async function main() {
  ...
  const cleaned = lines.filter((ln) => ln.trim() !== CURSOR_TRAILER);
  const subject = cleaned.find((ln) => ln.trim() !== '') || '';
  const rewrite = REWRITES[subject];

  if (rewrite) {
    const parts = Array.isArray(rewrite) ? rewrite : [rewrite];
    const out = [parts[0]];
    if (parts[1]) {
      out.push('', parts[1]);
    }
    for (const ln of cleaned) {
      if (ln.startsWith('Co-authored-by:') && ln.trim() !== CURSOR_TRAILER) {
  ...
  process.stdout.write(`${cleaned.join('\n').trimEnd()}\n`);
}
